...
Workflow:
1. Identify whether the goal is help, config validation, job listing, running all jobs, running one job, keeping one job listening for WhatsApp bridge events, opening the interactive menu, or publishing the binary.
...
8. Use `listen --config <path> --job <name>` when the task is specifically about waiting for WhatsApp personal channel events and re-running one selected job each time a new allowlisted message arrives.
9. Use `interactive` or `-i` only when the task explicitly calls for menu-driven or ad-hoc prompting behavior.
10. Summarize the actual command outcome, including any validation errors, selected job, listener state, logs path, or publish output path.

CLI contract:
- Supported commands are `run`, `listen`, `validate`, `jobs`, `interactive`, and `help`.
- `--job` is valid with `run` and required with `listen`.
